<template>
  <div class="my-container">
    ...
    <van-cell-group v-if="$store.state.user">
      <van-cell style="text-align: center;" title="退出登录" clickable @click="exit"/>
    </van-cell-group>
    <!-- /其它 -->
  </div>
</template>
...
export default {
  ...
  created () {
    // 判断
    if (this.$store.state.user) { // 页面创建 判断是否有token
    // 然后调用 获取用户个人信息
      this.showUser()
    }
  },
  ...
  methods: {
    ...
    async exit () { // 退出 弹窗 确认退出时设置commit为null
